...
import com.elfmcys.yesstevemodel.client.animation.custom.CustomRouletteGroup;
...
public class CustomRouletteEditorScreen extends Screen {
    ...
    public CustomRouletteEditorScreen(String modelId, ModelAssembly modelAssembly) {
        super(Component.translatable("gui.yes_steve_model.roulette.editor.title"));
    ...
    protected void init() {
        ...
        addRenderableWidget(new FlatColorButton(centerX - 150, bottomY, 60, 20,
                Component.translatable("gui.yes_steve_model.roulette.editor.new_group"), button -> createNewGroup()));

        // 删除分组
        addRenderableWidget(new FlatColorButton(centerX - 80, bottomY, 60, 20,
                Component.translatable("gui.yes_steve_model.roulette.editor.delete_group"), button -> deleteSelectedGroup()));

        // 保存
        addRenderableWidget(new FlatColorButton(centerX + 20, bottomY, 60, 20,
                Component.translatable("gui.yes_steve_model.roulette.editor.save"), button -> saveLayout()));

        // 恢复原始
        addRenderableWidget(new FlatColorButton(centerX + 90, bottomY, 80, 20,
                Component.translatable("gui.yes_steve_model.roulette.editor.revert"), button -> revertLayout()));
    }
    ...
    private void saveLayout() {
        ...
        if (player != null) {
            player.sendSystemMessage(Component.translatable("message.yes_steve_model.roulette.editor.saved"));
        }
        onClose();
    }
    ...
    private void revertLayout() {
        ...
        LocalPlayer player = minecraft != null ? minecraft.player : null;
        if (player != null) {
            player.sendSystemMessage(Component.translatable("message.yes_steve_model.roulette.editor.reverted"));
        }
        onClose();
    }
    ...
    public void extractRenderState(GuiGraphicsExtractor extractor, int mouseX, int mouseY, float partialTick) {
        ...
        // 标题
        g.centeredText(this.font, Component.translatable("gui.yes_steve_model.roulette.editor.title"), this.width / 2, 8, 0xFFFFFF);
        ...
        // 左面板标题
        g.text(this.font, Component.translatable("gui.yes_steve_model.roulette.editor.available"), leftX, topY - 2, 0xFFAAAAAA, true);
        // 右面板标题
        g.text(this.font, Component.translatable("gui.yes_steve_model.roulette.editor.custom_layout"), rightX, topY - 2, 0xFFAAAAAA, true);
