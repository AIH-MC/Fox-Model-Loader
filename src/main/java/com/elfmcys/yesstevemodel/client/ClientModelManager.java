...
public class ClientModelManager {
    ...
    public static void loadDefaultModel() {
        ...
        try {
            String resourceFile = "assets/yes_steve_model/builtin/default/ysm.json";
            URL resourceUrl = YesSteveModel.class.getClassLoader().getResource(resourceFile);
            if (resourceUrl == null) {
                YesSteveModel.LOGGER.error("[YSM] Builtin default model not found in classpath: " + resourceFile);
                return;
            }
            ...
            if ("jar".equals(uri.getScheme())) {
                ...
                URI fileSystemUri = separator >= 0 ? URI.create(uriText.substring(0, separator + 2)) : uri;
                FileSystem jarFs;
                try {
                    jarFs = FileSystems.getFileSystem(fileSystemUri);
                } catch (FileSystemNotFoundException e) {
                    jarFs = FileSystems.newFileSystem(fileSystemUri, Collections.emptyMap());
                }
                defaultPath = jarFs.getPath("/" + resourceFile).getParent();
            } else {
