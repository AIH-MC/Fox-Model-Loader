...
public final class ModelRepoClient {
    ...
    public static List<ModelRepoEntry> list(String sourceUrl, ResourceStationConfig.State config) throws Exception {
        ...
        if (debugLogEnabled()) {
            YesSteveModel.LOGGER.info("[YSM][ResourceStation] Listing source started sourceUrl={} timeoutMs={} maxDownloadBytes={} mainlandChinaMode={} githubAccelerators={}",
        ...
        try {
            ...
            if (debugLogEnabled()) {
                YesSteveModel.LOGGER.warn("[YSM][ResourceStation] Listing source failed sourceUrl={} elapsedMs={}", sourceUrl, elapsedMs(started), e);
            }
            throw e;
        }
    }
    ...
    public static byte[] download(ModelRepoEntry entry, ResourceStationConfig.State config) throws IOException {
        ...
        if (debugLogEnabled()) {
            YesSteveModel.LOGGER.info("[YSM][ResourceStation] Download candidates entry={} fileName={} githubFile={} candidates={}",
    ...
    private static List<ModelRepoEntry> parseIndex(String indexUrl, String json) {
        ...
        for (JsonElement element : array) {
            ...
            if (url == null || url.isBlank()) {
                if (debugLogEnabled()) {
                    YesSteveModel.LOGGER.warn("[YSM][ResourceStation] Skipping index entry without download URL indexUrl={} name={}", indexUrl, name);
                }
                continue;
            }
        ...
        if (debugLogEnabled()) {
            YesSteveModel.LOGGER.info("[YSM][ResourceStation] Parsed index indexUrl={} rawEntries={} importableEntries={}", indexUrl, array.size(), entries.size());
        }
        return entries;
    }
    ...
    private static List<ModelRepoEntry> listGithub(URI uri, ResourceStationConfig.State config) throws Exception {
        ...
        if (debugLogEnabled()) {
            YesSteveModel.LOGGER.info("[YSM][ResourceStation] GitHub listing owner={} repo={} branch={} path={}",
        ...
        try {
            ...
            YesSteveModel.LOGGER.warn("[YSM] GitHub tree listing failed for {}/{}@{}, trying archive fallback",
                    path.owner(), path.repo(), path.branch(), treeError);
            listGithubArchive(path.owner(), path.repo(), path.branch(), path.path(), config, entries);
        }
        if (debugLogEnabled()) {
            YesSteveModel.LOGGER.info("[YSM][ResourceStation] GitHub listing finished owner={} repo={} branch={} path={} entries={}",
    ...
    private static void listGithubArchive(String owner, String repo, String branch, String path, ResourceStationConfig.State config, List<ModelRepoEntry> entries) throws IOException {
        ...
        if (debugLogEnabled()) {
            YesSteveModel.LOGGER.info("[YSM][ResourceStation] GitHub archive fallback finished owner={} repo={} branch={} path={} entries={}",
    ...
    private static JsonElement readGithubJson(String url, ResourceStationConfig.State config, int maxBytes) throws IOException {
        ...
        if (debugLogEnabled()) {
            YesSteveModel.LOGGER.info("[YSM][ResourceStation] GitHub JSON candidates sourceUrl={} candidates={}", url, candidates);
        }
        for (String candidate : candidates) {
            try {
                ...
                last = e;
                YesSteveModel.LOGGER.warn("[YSM] GitHub JSON candidate failed: {}", candidate, e);
            }
        }
    ...
    private static byte[] readGithub(String url, ResourceStationConfig.State config, int maxBytes) throws IOException {
        ...
        List<String> candidates = githubCandidates(url, config);
        if (debugLogEnabled()) {
            YesSteveModel.LOGGER.info("[YSM][ResourceStation] GitHub binary candidates sourceUrl={} candidates={}", url, candidates);
        }
        for (String candidate : candidates) {
            try {
    ...
    private static byte[] read(String url, int timeoutMs, int maxBytes, ProgressListener listener, long expectedBytes, boolean bypassCache) throws IOException {
        ...
        try {
            ...
            if (debugLogEnabled()) {
                YesSteveModel.LOGGER.info("[YSM][ResourceStation] HTTP response id={} code={} contentLength={} contentType={} elapsedMs={}",
            ...
            try (InputStream in = connection.getInputStream(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
                ...
                if (debugLogEnabled()) {
                    YesSteveModel.LOGGER.info("[YSM][ResourceStation] HTTP request finished id={} bytes={} elapsedMs={}",
            ...
            if (debugLogEnabled()) {
                YesSteveModel.LOGGER.warn("[YSM][ResourceStation] HTTP request failed id={} url={} elapsedMs={}",
    ...
    private static boolean debugLogEnabled() {
        return GeneralConfig.safeGet(GeneralConfig.RESOURCE_STATION_DEBUG_LOG, false);
    }
    ...
    private static List<String> rankCandidates(List<String> candidates, ResourceStationConfig.State config) {
        ...
        if (debugLogEnabled()) {
            YesSteveModel.LOGGER.info("[YSM][ResourceStation] Download ranked candidates={}", ranked);
        }
        return ranked;
    }
    ...
    private static CandidateProbe probeCandidate(String url, ResourceStationConfig.State config, int index) {
        ...
        try {
            ...
            if (debugLogEnabled()) {
                YesSteveModel.LOGGER.info("[YSM][ResourceStation] Probe candidate ok url={} bytes={} elapsedMs={} speed={}",
            ...
            if (debugLogEnabled()) {
                YesSteveModel.LOGGER.info("[YSM][ResourceStation] Probe candidate failed url={} error={}", url, e.toString());
            }
