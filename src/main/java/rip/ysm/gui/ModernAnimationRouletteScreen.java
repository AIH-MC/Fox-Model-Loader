...
public class ModernAnimationRouletteScreen extends Screen {
    ...
    private void renderLabels(GuiGraphics g) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            boolean showKey = page() == 0 && navigationStack.size() == 1 && absoluteIdx < ExtraAnimationKey.getKeyMappings().size();
    ...
    private void renderKeyBinding(GuiGraphics g, int slot, int x, int y) {
        if (slot >= ExtraAnimationKey.getKeyMappings().size()) return;
        KeyMapping km = ExtraAnimationKey.getKeyMappings().get(slot);
